...
public class NettyTcpClient implements IMSClientInterface {
    ...
    public void init(Vector<String> serverUrlList, OnEventListener listener, IMSConnectStatusCallback callback) {
        ...
        isClosed = false;
        if (mImOptions == null) {
            mImOptions = IMOptions.getDefault();
        }
     ...
     */
    @Override
    public void sendMsg(ImData msg) {
     ...
     */
    @Override
    public void sendMsg(ImData msg, boolean isJoinTimeoutManager) {
        if (msg == null) {
        ...
        try {
            appendText(mContext, "发送消息成\n");

            channel.writeAndFlush(msg.parse());
        } catch (Exception ex) {

            appendText(mContext, "发送消息失败，reason:" + ex.getMessage() + "\tmessage=" + msg + "\n");
    ...
    public static void appendText(Context context, String text) {

        String path = context.getExternalCacheDir() + "/im";
        File logFile = createFile(path, "imlog");

        if (logFile != null) {
            appendText(logFile.getAbsolutePath(), millis2String(System.currentTimeMillis(), getDefaultFormat()) + text);
        }
...
//            heartbeatInterval = mImOptions.getForegroundHeartbeatInterval();
//        } else if (this.appStatus == IMSConfig.APP_STATUS_BACKGROUND) {
//            heartbeatInterval = mImOptions.getBackgroundHeartbeatInterval();
//        }
    ...
    private void initBootstrap() {
        ...
        // 设置连接超时时长
        bootstrap.option(ChannelOption.CONNECT_TIMEOUT_MILLIS, mImOptions.getConnectTimeout());
        // 设置初始化Channel
    ...
    private void onConnectStatusCallback(int connectStatus) {
        ...
        switch (connectStatus) {
            ...
            case IMSConfig.CONNECT_STATE_SUCCESSFUL: {
                ...
                // 连接成功，发送握手消息
                ImData handshakeMsg = getHandshakeMsg();
...
//            }
            // 3次心跳没响应，代表连接已断开,写心跳采用alarm manage
//            channel.pipeline().addFirst(IdleStateHandler.class.getSimpleName(), new IdleStateHandler(
//                    mImOptions.getHeartbeatInterval() * 3, 0, 0, TimeUnit.MILLISECONDS));
...
////            }
    ...
    public void sendHeartMessage() {
        ...
        ImData heartbeatMsg = getHeartbeatMsg();
...
//                    removeHandler(HeartbeatHandler.class.getSimpleName());
        ...
        private int connectServer() {
            ...
            for (int i = 0; (!isClosed && i < serverUrlList.size()); i++) {
                ...
                for (int j = 1; j <= IMSConfig.DEFAULT_RECONNECT_COUNT; j++) {
                    ...
                    System.out.println(String.format("正在进行『%s』的第『%d』次连接，当前重连延时时长为『%dms』", serverUrl, j, j * mImOptions.getReconnectInterval()));
                    ...
                    try {
                        ...
                        if (channel != null) {
                            ...
                            // 连接失败，则线程休眠n * 重连间隔时长
                            Thread.sleep(j * mImOptions.getReconnectInterval());
                        }
                    } catch (InterruptedException e) {
                        close();
                        break;// 线程被中断，则强制关闭
    ...
    public boolean isHandShakeMsg(Object msg) {
        if (mOnEventListener != null) {
            return mOnEventListener.isHandShakeMsg(msg);
        }
        return false;
    }
    ...
    public boolean isAuthAgree(Object msg) {
        if (mOnEventListener != null) {
            return mOnEventListener.isAuthAgree(msg);
        }
        return false;
    }
    ...
    public boolean isHeartbeatMsg(Object msg) {
        ...
        if (mOnEventListener != null) {
            return mOnEventListener.isHeartbeatMsg(msg);
        }
        return false;
    }
