                ...
                .subscribe(new Observer<ApiResponse<ResultType>>() {
                    ...
                    public void onNext(final ApiResponse<ResultType> response) {
                        if (response.isSuccessful()) {
                            mAppExecutors.diskIO().execute(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    mAppExecutors.mainThread().execute(new Runnable() {
                                        @Override
                                        public void run() {
                                            Resource<ResultType> res = Resource.success(processResponse(response), response.message);
                                            result.setValue(res);
                            ...
                            result.setValue(Resource.error(response.message, processResponse(response), response.code));
                            result.setValue((Resource<ResultType>) Resource.complete(null));
                        }
                    }
