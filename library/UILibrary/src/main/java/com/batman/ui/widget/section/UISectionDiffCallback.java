...
public class UISectionDiffCallback<H extends UISection.Model<H>, T extends UISection.Model<T>>
        extends DiffUtil.Callback {

    private ArrayList<UISection<H, T>> mOldList = new ArrayList<>();
    private ArrayList<UISection<H, T>> mNewList = new ArrayList<>();
    ...
    public UISectionDiffCallback(
            @Nullable List<UISection<H, T>> oldList,
            @Nullable List<UISection<H, T>> newList) {
        if (oldList != null) {
    ...
    private void generateIndex(List<UISection<H, T>> list,
        ...
        for (int i = 0; i < list.size(); i++) {
            UISection<H, T> section = list.get(i);
            if (section.isLocked()) {
                continue;
            }
        ...
        if (list.isEmpty()) {
            ...
            UISection lastSection = list.get(list.size() - 1);
     ...
     */
    protected void onGenerateCustomIndexBeforeSectionList(IndexGenerationInfo generationInfo, List<UISection<H, T>> list) {
     ...
     * @param list           the whole list info
     */
    protected void onGenerateCustomIndexAfterSectionList(IndexGenerationInfo generationInfo, List<UISection<H, T>> list) {
     ...
     */
    protected void onGenerateCustomIndexBeforeItemList(IndexGenerationInfo generationInfo, UISection<H, T> section, int sectionIndex) {
     ...
     * @param sectionIndex   section index info
     */
    protected void onGenerateCustomIndexAfterItemList(IndexGenerationInfo generationInfo, UISection<H, T> section, int sectionIndex) {
     ...
     */
    protected boolean areCustomContentsTheSame(@Nullable UISection<H, T> oldSection, int oldItemIndex,
                                                   @Nullable UISection<H, T> newSection, int newItemIndex) {
        return false;
    }
    ...
    public boolean areItemsTheSame(int oldItemPosition, int newItemPosition) {
        ...
        UISection<H, T> oldModel = mOldList.get(oldSectionIndex);
        UISection<H, T> newModel = mNewList.get(newSectionIndex);
    ...
    public boolean areContentsTheSame(int oldItemPosition, int newItemPosition) {
        ...
        if (oldItemIndex == ITEM_INDEX_LOAD_BEFORE || oldItemIndex == ITEM_INDEX_LOAD_AFTER) {
            ...
            // in UIStickySectionAdapter.onViewAttachedToWindow
            return false;
        }

        if (UISection.isCustomItemIndex(oldItemIndex)) {
    ...
    public static class IndexGenerationInfo {
        ...
        public final void appendCustomIndex(int sectionIndex, int itemIndex) {
            ...
            int offset = UISection.ITEM_INDEX_CUSTOM_OFFSET + itemIndex;
            if(!UISection.isCustomItemIndex(offset)){
        ...
        private final void appendWholeListIndex(int itemIndex) {
            sectionIndexArray.append(currentPosition, UISection.SECTION_INDEX_UNKNOWN);
            itemIndexArray.append(currentPosition, itemIndex);
            currentPosition++;
        }
    }
