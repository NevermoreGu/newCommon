...
public class UIBottomSheetListItemDecoration extends RecyclerView.ItemDecoration
        implements IUISkinHandlerDecoration {

    private final Paint mSeparatorPaint;
    private final int mSeparatorAttr;

    public UIBottomSheetListItemDecoration(Context context) {
        mSeparatorPaint = new Paint();
        mSeparatorPaint.setStrokeWidth(1);
        mSeparatorPaint.setStyle(Paint.Style.STROKE);
        mSeparatorAttr = R.attr.ui_skin_support_bottom_sheet_separator_color;
        if (mSeparatorAttr != 0) {
            mSeparatorPaint.setColor(UIResHelper.getAttrColor(context, mSeparatorAttr));
        }
    }
    ...
    public void onDrawOver(@NonNull Canvas c, @NonNull RecyclerView parent, @NonNull RecyclerView.State state) {
        ...
        for (int i = 0; i < parent.getChildCount(); i++) {
            ...
            if (view instanceof UIBottomSheetListItemView) {
                if (position > 0 &&
                        adapter.getItemViewType(position - 1) != UIBottomSheetListAdapter.ITEM_TYPE_NORMAL) {
    ...
    public void handle(@NonNull RecyclerView recyclerView,
        ...
        if (mSeparatorAttr != 0) {
            mSeparatorPaint.setColor(UIResHelper.getAttrColor(theme, mSeparatorAttr));
        }
    }
}
