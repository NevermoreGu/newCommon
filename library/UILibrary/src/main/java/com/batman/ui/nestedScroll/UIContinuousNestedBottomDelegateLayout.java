...
public abstract class UIContinuousNestedBottomDelegateLayout extends UIFrameLayout implements
        NestedScrollingChild2, NestedScrollingParent2, IUIContinuousNestedBottomView {
    public static final String KEY_SCROLL_INFO_OFFSET = "@ui_scroll_info_bottom_dl_offset";

    private final NestedScrollingParentHelper mParentHelper;
    private final NestedScrollingChildHelper mChildHelper;
    private View mHeaderView;
    private View mContentView;
    private UIViewOffsetHelper mHeaderViewOffsetHelper;
    private UIViewOffsetHelper mContentViewOffsetHelper;
    ...
    public UIContinuousNestedBottomDelegateLayout(Context context) {
        this(context, null);
    }

    public UIContinuousNestedBottomDelegateLayout(Context context, AttributeSet attrs) {
        this(context, null, 0);
    }

    public UIContinuousNestedBottomDelegateLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (!(mContentView instanceof IUIContinuousNestedBottomView)) {
            throw new IllegalStateException("the view create by onCreateContentView() " +
                    "should implement from IUIContinuousNestedBottomView");
        }
        addView(mHeaderView, new LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT, getHeaderHeightLayoutParam()));
        addView(mContentView, new LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));
        mHeaderViewOffsetHelper = new UIViewOffsetHelper(mHeaderView);
        mContentViewOffsetHelper = new UIViewOffsetHelper(mContentView);
    ...
    private int getMiniOffset() {
        IUIContinuousNestedBottomView b = (IUIContinuousNestedBottomView) mContentView;
        int contentHeight = b.getContentHeight();
        LayoutParams headerLp = (LayoutParams) mHeaderView.getLayoutParams();
    ...
    public int getContentHeight() {
        ...
        if (bc == IUIContinuousNestedBottomView.HEIGHT_IS_ENOUGH_TO_SCROLL || bc > mContentView.getHeight()) {
            return IUIContinuousNestedBottomView.HEIGHT_IS_ENOUGH_TO_SCROLL;
        }
    ...
    public void checkLayout() {
        ...
        IUIContinuousNestedBottomView bottomView = (IUIContinuousNestedBottomView) mContentView;
    ...
    private int offsetBy(int dyUnConsumed) {
        ...
        mOnScrollNotifier.notify(-mHeaderViewOffsetHelper.getTopAndBottomOffset(),
                mHeaderView.getHeight() + ((IUIContinuousNestedBottomView) mContentView).getScrollOffsetRange());
    ...
    public void consumeScroll(int dy) {
        if (dy == Integer.MAX_VALUE) {
            offsetBy(dy);
            ((IUIContinuousNestedBottomView) mContentView).consumeScroll(Integer.MAX_VALUE);
            return;
        } else if (dy == Integer.MIN_VALUE) {
            ((IUIContinuousNestedBottomView) mContentView).consumeScroll(Integer.MIN_VALUE);
            offsetBy(dy);
            return;
        }
        ((IUIContinuousNestedBottomView) mContentView).consumeScroll(dy);
    }

    @Override
    public void smoothScrollYBy(int dy, int duration) {
        ((IUIContinuousNestedBottomView) mContentView).smoothScrollYBy(dy, duration);
    }

    @Override
    public void stopScroll() {
        ((IUIContinuousNestedBottomView) mContentView).stopScroll();
    }

    @Override
    public int getCurrentScroll() {
        return -mHeaderViewOffsetHelper.getTopAndBottomOffset() +
                ((IUIContinuousNestedBottomView) mContentView).getCurrentScroll();
    }
    ...
    public int getScrollOffsetRange() {
        ...
        return mHeaderView.getHeight() - getHeaderStickyHeight() +
                ((IUIContinuousNestedBottomView) mContentView).getScrollOffsetRange();
    }
    ...
    public void injectScrollNotifier(final OnScrollNotifier notifier) {
        ...
        if (mContentView instanceof IUIContinuousNestedBottomView) {
            ((IUIContinuousNestedBottomView) mContentView).injectScrollNotifier(new OnScrollNotifier() {
                @Override
    ...
    public void saveScrollInfo(@NonNull Bundle bundle) {
        ...
        if (mContentView != null) {
            ((IUIContinuousNestedBottomView) mContentView).saveScrollInfo(bundle);
        }
    }
    ...
    public void restoreScrollInfo(@NonNull Bundle bundle) {
        ...
        offset = UILangHelper.constrain(offset, getMiniOffset(), 0);
        mHeaderViewOffsetHelper.setTopAndBottomOffset(offset);
        mContentViewOffsetHelper.setTopAndBottomOffset(offset);
        if (mContentView != null) {
            ((IUIContinuousNestedBottomView) mContentView).restoreScrollInfo(bundle);
        }
    }
    ...
    private boolean isPointInHeaderBounds(int x, int y) {
        UIViewHelper.getDescendantRect(this, mHeaderView, mTempRect);
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (isBeingDragged) {
                    lastMotionY = y;
                    if (dy < 0 && ((IUIContinuousNestedBottomView) mContentView).getCurrentScroll() > 0) {
    ...
    class ViewFlinger implements Runnable {
        ...
        private void internalPostOnAnimation() {
            ...
            ViewCompat.postOnAnimation(UIContinuousNestedBottomDelegateLayout.this, this);
