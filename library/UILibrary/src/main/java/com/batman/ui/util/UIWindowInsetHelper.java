...
public class UIWindowInsetHelper {
    ...
    public UIWindowInsetHelper(ViewGroup viewGroup, IWindowInsetLayout windowInsetLayout) {
        ...
        KEYBOARD_HEIGHT_BOUNDARY = UIDisplayHelper.dp2px(viewGroup.getContext(), KEYBOARD_HEIGHT_BOUNDARY_DP);
    ...
    public boolean defaultApplySystemWindowInsets19(ViewGroup viewGroup, Rect insets) {
        ...
        if (insets.bottom >= KEYBOARD_HEIGHT_BOUNDARY && shouldInterceptKeyboardInset(viewGroup)) {
            if(viewGroup instanceof IWindowInsetKeyboardConsumer){
                ...
                UIViewHelper.setPaddingBottom(viewGroup, insets.bottom);
            }
            viewGroup.setTag(R.id.ui_window_inset_keyboard_area_consumer, KEYBOARD_CONSUMER);
            insets.bottom = 0;
        } else {
            viewGroup.setTag(R.id.ui_window_inset_keyboard_area_consumer, null);
            if(viewGroup instanceof IWindowInsetKeyboardConsumer){
    ...
    public static View findKeyboardAreaConsumer(@NonNull View view) {
        while (view != null) {
            Object tag = view.getTag(R.id.ui_window_inset_keyboard_area_consumer);
