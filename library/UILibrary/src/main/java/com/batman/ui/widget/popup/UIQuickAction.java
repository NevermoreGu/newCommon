...
import com.batman.ui.widget.UIRadiusImageView2;

import java.util.ArrayList;
import java.util.Objects;


public class UIQuickAction extends UINormalPopup<UIQuickAction> {
    ...
    public UIQuickAction(Context context, int width, int height) {
        super(context, width, height);
        mActionHeight = height;
        mMoreArrowWidth = UIResHelper.getAttrDimen(context, R.attr.ui_quick_action_more_arrow_width);
        mPaddingHor = UIResHelper.getAttrDimen(context, R.attr.ui_quick_action_padding_hor);
    }

    public UIQuickAction moreArrowWidth(int moreArrowWidth) {
        mMoreArrowWidth = moreArrowWidth;
        return this;
    }

    public UIQuickAction paddingHor(int paddingHor) {
        mPaddingHor = paddingHor;
        return this;
    }

    public UIQuickAction actionWidth(int actionWidth) {
        mActionWidth = actionWidth;
        return this;
    }

    public UIQuickAction actionHeight(int actionHeight) {
        mActionHeight = actionHeight;
        return this;
    }

    public UIQuickAction addAction(Action action) {
        mActions.add(action);
        return this;
    }

    public UIQuickAction showMoreArrowIfNeeded(boolean showMoreArrowIfNeeded) {
    ...
    @Override
    public UIQuickAction show(@NonNull View anchor) {
    ...
    protected AppCompatImageView createMoreArrowView(boolean isLeft) {
        UIRadiusImageView2 arrowView = new UIRadiusImageView2(mContext);
        UISkinValueBuilder builder = UISkinValueBuilder.acquire();
        if (isLeft) {
            arrowView.setPadding(mPaddingHor, 0, 0, 0);
            builder.src(R.attr.ui_skin_support_quick_action_more_left_arrow);
        } else {
            arrowView.setPadding(0, 0, mPaddingHor, 0);
            builder.src(R.attr.ui_skin_support_quick_action_more_right_arrow);
        }
        builder.tintColor(R.attr.ui_skin_support_quick_action_more_tint_color);
        ...
        UISkinHelper.setSkinValue(arrowView, builder);
    ...
    private class Adapter extends ListAdapter<Action, VH> implements VH.Callback {
        ...
        public void onClick(View v, int adapterPosition) {
            ...
            if (onClickListener != null) {
                onClickListener.onClick(UIQuickAction.this, action, adapterPosition);
            }
        }
    ...
    public static class Action {
        ...
        int iconAttr = 0;
        int textColorAttr = R.attr.ui_skin_support_quick_action_item_tint_color;
        int iconTintColorAttr = R.attr.ui_skin_support_quick_action_item_tint_color;
    ...
    public interface OnClickListener {
        void onClick(UIQuickAction quickAction, Action action, int position);
    }
    ...
    public static class DefaultItemView extends ItemView {
        ...
        public DefaultItemView(Context context, AttributeSet attrs) {
            super(context, attrs);
            int paddingHor = UIResHelper.getAttrDimen(
                    context, R.attr.ui_quick_action_item_padding_hor);
            int paddingVer = UIResHelper.getAttrDimen(
                    context, R.attr.ui_quick_action_item_padding_ver);
            setPadding(paddingHor, paddingVer, paddingHor, paddingVer);
            mIconView = new AppCompatImageView(context);
            mIconView.setId(UIViewHelper.generateViewId());
            mTextView = new TextView(context);
            mTextView.setId(UIViewHelper.generateViewId());
            ...
            textLp.topMargin = UIResHelper.getAttrDimen(
                    context, R.attr.ui_quick_action_item_middle_space);
