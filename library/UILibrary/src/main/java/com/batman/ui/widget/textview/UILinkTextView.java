...
package com.batman.ui.widget.textview;
...
import com.batman.ui.alpha.UIAlphaTextView;
import com.batman.ui.link.UILinkTouchMovementMethod;
import com.batman.ui.link.UILinkify;
import com.batman.ui.span.UIOnSpanClickListener;
 ...
 * {@link UILinkTextView} 有以下特点:
 * <ul>
 * <li>可以通过 {@link com.batman.ui.R.styleable#UILinkTextView} 中的属性设置链接的样式</li>
 * <li>可以通过 {@link UILinkTextView#setOnLinkClickListener(OnLinkClickListener)} 设置链接的点击事件，
 ...
 * @date 2017-03-17
 */
public class UILinkTextView extends UIAlphaTextView implements UIOnSpanClickListener {
    private static final String TAG = "LinkTextView";
    private static final int MSG_CHECK_DOUBLE_TAP_TIMEOUT = 1000;
    public static int AUTO_LINK_MASK_REQUIRED = UILinkify.PHONE_NUMBERS | UILinkify.EMAIL_ADDRESSES | UILinkify.WEB_URLS;
    ...
    public UILinkTextView(Context context) {
        this(context, null);
        mLinkBgColor = null;
        mLinkTextColor = ContextCompat.getColorStateList(context, R.color.ui_s_link_color);
    }

    public UILinkTextView(Context context, ColorStateList linkTextColor, ColorStateList linkBgColor) {
        this(context, null);
        mLinkBgColor = linkBgColor;
        mLinkTextColor = linkTextColor;
    }

    public UILinkTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.UILinkTextView);
        mLinkBgColor = array.getColorStateList(R.styleable.UILinkTextView_ui_linkBackgroundColor);
        mLinkTextColor = array.getColorStateList(R.styleable.UILinkTextView_ui_linkTextColor);
        array.recycle();
        if (mOriginText != null) {
            setText(mOriginText);
        }
    ...
    public void setText(CharSequence text, BufferType type) {
        ...
        if (!TextUtils.isEmpty(text)) {
            ...
            UILinkify.addLinks(builder, mAutoLinkMaskCompat, mLinkTextColor, mLinkBgColor, this);
            text = builder;
        }
