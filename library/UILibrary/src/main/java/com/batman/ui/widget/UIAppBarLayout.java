 ...
 * and {@link UICollapsingTopBarLayout}
 * <p>
...
public class UIAppBarLayout extends AppBarLayout implements IWindowInsetLayout {
    public UIAppBarLayout(Context context) {
        super(context);
    }

    public UIAppBarLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    public boolean applySystemWindowInsets19(final Rect insets) {
        if (ViewCompat.getFitsSystemWindows(this)) {
            ...
            for (int i = 0; i < getChildCount(); i++) {
                ...
                if (UIWindowInsetHelper.jumpDispatch(child)) {
                    continue;
                }


                if (!UIWindowInsetHelper.isHandleContainer(child)) {
