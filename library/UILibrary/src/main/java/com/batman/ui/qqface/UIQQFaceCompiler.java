...
/**
 * {@link UIQQFaceView} 的内容解析器，将文本内容解析成 {@link UIQQFaceView} 想要的数据格式。
 *
 * @author cginechen
 * @date 2016-12-21
 */

public class UIQQFaceCompiler {
    private static final int SPAN_COLUMN = 2;
    private static final Map<IUIQQFaceManager, UIQQFaceCompiler> sInstanceMap = new HashMap<>(4);
    private static IUIQQFaceManager sDefaultQQFaceManager = new UINoQQFaceManager();

    public static void setDefaultQQFaceManager(@NonNull IUIQQFaceManager defaultQQFaceManager) {
        sDefaultQQFaceManager = defaultQQFaceManager;
    }

    private LruCache<CharSequence, ElementList> mCache;
    private IUIQQFaceManager mQQFaceManager;


    @MainThread
    public static UIQQFaceCompiler getDefaultInstance(){
        return getInstance(sDefaultQQFaceManager);
    }

    @MainThread
    public static UIQQFaceCompiler getInstance(IUIQQFaceManager manager) {
        UIQQFaceCompiler instance = sInstanceMap.get(manager);
        if (instance != null) {
            return instance;
        }
        instance = new UIQQFaceCompiler(manager);
        sInstanceMap.put(manager, instance);
        return instance;
    }

    private UIQQFaceCompiler(IUIQQFaceManager manager) {
    ...
    private ElementList compile(CharSequence text, int start, int end, boolean inSpan) {
        ...
        if (!inSpan && (text instanceof Spannable)) {
            ...
            spans = ((Spannable) text).getSpans(
                    0,
                    text.length() - 1,
                    UITouchableSpan.class);
            Arrays.sort(spans, new Comparator<UITouchableSpan>() {
                @Override
                public int compare(UITouchableSpan o1, UITouchableSpan o2) {
    ...
    private ElementList realCompile(CharSequence text, int start, int end, UITouchableSpan[] spans, int[] spanInfo) {
        int size = text.length();
        int nearSpanIndex = -1;
    ...
    public static class Element {
        ...
        private UITouchableSpan mTouchableSpan;
        ...
        public UITouchableSpan getTouchableSpan() {
        ...
        public static Element createTouchSpanElement(CharSequence text,
                                                     UITouchableSpan touchableSpan,
                                                     UIQQFaceCompiler compiler) {
            Element element = new Element();
