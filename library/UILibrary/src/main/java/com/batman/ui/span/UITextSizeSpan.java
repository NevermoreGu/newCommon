 ...
 * 无法根据具体需求进行底部对齐或者顶部对齐。而 UITextSizeSpan 则可以多传一个参数，让你可以根据具体情况来决定偏移值。
 *
 * @author cginechen
 * @date 2016-12-02
 */

public class UITextSizeSpan extends ReplacementSpan {
    private int mTextSize;
    private int mVerticalOffset;
    private Paint mPaint;
    private Typeface mTypeface;

    public UITextSizeSpan(int textSize, int verticalOffset){
       this(textSize, verticalOffset, null);
    }

    public UITextSizeSpan(int textSize, int verticalOffset, Typeface typeface){
        mTextSize = textSize;
