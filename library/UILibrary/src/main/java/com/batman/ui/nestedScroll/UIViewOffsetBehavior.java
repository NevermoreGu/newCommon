...
public class UIViewOffsetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {

    private UIViewOffsetHelper viewOffsetHelper;

    private int tempTopBottomOffset = 0;
    private int tempLeftRightOffset = 0;

    public UIViewOffsetBehavior() {
    }

    public UIViewOffsetBehavior(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, V child, int layoutDirection) {
        ...
        if (viewOffsetHelper == null) {
            viewOffsetHelper = new UIViewOffsetHelper(child);
        }
