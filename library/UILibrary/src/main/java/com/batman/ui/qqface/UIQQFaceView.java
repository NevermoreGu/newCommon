...
import com.batman.ui.span.UITouchableSpan;
 ...
 * <li>支持与 {@link UITouchableSpan} 配合使用实现内容可点击。</li>
 * </ul>
 *
 * @author cginechen
 * @date 2016-12-21
...
public class UIQQFaceView extends View {
    private static final String TAG = "UIQQFaceView";
    private CharSequence mOriginText;
    private UIQQFaceCompiler.ElementList mElementList;
    private UIQQFaceCompiler mCompiler;
    ...
    private HashMap<UIQQFaceCompiler.Element, SpanInfo> mSpanInfos = new HashMap<>();
    ...
    public UIQQFaceView(Context context) {
        this(context, null);
    }

    public UIQQFaceView(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.UIQQFaceStyle);
    }

    public UIQQFaceView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray array = getContext().obtainStyledAttributes(attrs,
                R.styleable.UIQQFaceView, defStyleAttr, 0);
        mQQFaceSizeAddon = -UIDisplayHelper.dp2px(context, 2); // 默认表情小一点好看
        mTextSize = array.getDimensionPixelSize(R.styleable.UIQQFaceView_android_textSize,
                UIDisplayHelper.dp2px(context, 14));
        mTextColor = array.getColorStateList(R.styleable.UIQQFaceView_android_textColor);
        mIsSingleLine = array.getBoolean(R.styleable.UIQQFaceView_android_singleLine, false);
        mMaxLine = array.getInt(R.styleable.UIQQFaceView_android_maxLines, mMaxLine);
        int lineSpace = array.getDimensionPixelOffset(R.
                styleable.UIQQFaceView_android_lineSpacingExtra, 0);
        setLineSpace(lineSpace);
        int ellipsize = -1;
        ellipsize = array.getInt(R.styleable.UIQQFaceView_android_ellipsize, ellipsize);
        switch (ellipsize) {
            case 1:
        ...
        mMaxWidth = array.getDimensionPixelSize(R.styleable.UIQQFaceView_android_maxWidth, mMaxWidth);
        mSpecialDrawablePadding = array.getDimensionPixelSize(R.styleable.UIQQFaceView_ui_special_drawable_padding, 0);
        final String text = array.getString(R.styleable.UIQQFaceView_android_text);
        if (!UILangHelper.isNullOrEmpty(text)) {
            mOriginText = text;
        }
        mMoreActionText = array.getString(R.styleable.UIQQFaceView_ui_more_action_text);
        mMoreActionColor = array.getColorStateList(R.styleable.UIQQFaceView_ui_more_action_color);
        mMoreActionBgColor = array.getColorStateList(R.styleable.UIQQFaceView_ui_more_action_bg_color);
        ...
        setCompiler(UIQQFaceCompiler.getDefaultInstance());
    }
    ...
    public void setCompiler(UIQQFaceCompiler compiler) {
    ...
    private void measureMoreActionTextLength() {
        if (UILangHelper.isNullOrEmpty(mMoreActionText)) {
    ...
    private void setText(CharSequence charSequence, boolean compareOldText) {
        if (compareOldText && UILangHelper.objectEquals(charSequence, mOriginText)) {
            return;
        }
        ...
        mSpanInfos.clear();
        if (UILangHelper.isNullOrEmpty(mOriginText)) {
            mElementList = null;
            requestLayout();
            invalidate();
            return;
        ...
        if (mOpenQQFace && mCompiler != null) {
            ...
            List<UIQQFaceCompiler.Element> elements = mElementList.getElements();
            if (elements != null) {
                for (int i = 0; i < elements.size(); i++) {
                    UIQQFaceCompiler.Element element = elements.get(i);
                    if (element.getType() == UIQQFaceCompiler.ElementType.SPAN) {
                        mSpanInfos.put(element, new SpanInfo(element.getTouchableSpan()));
                    }
                }
            }
        } else {
            mElementList = new UIQQFaceCompiler.ElementList(0, mOriginText.length());
            String[] strings = mOriginText.toString().split("\\n");
            for (int i = 0; i < strings.length; i++) {
                mElementList.add(UIQQFaceCompiler.Element.createTextElement(strings[i]));
                if (i != strings.length - 1) {
                    mElementList.add(UIQQFaceCompiler.Element.createNextLineElement());
                }
            }
        }
        mNeedReCalculateLines = true;
    ...
    private void calculateLinesInner(List<UIQQFaceCompiler.Element> elements, int limitWidth) {
        UIQQFaceCompiler.Element element;
        ...
        for (int i = 0; i < elements.size(); i++) {
            ...
            if (element.getType() == UIQQFaceCompiler.ElementType.DRAWABLE) {
            ...
            } else if (element.getType() == UIQQFaceCompiler.ElementType.TEXT) {
                CharSequence text = element.getText();
                measureText(text, widthStart, widthEnd);
            } else if (element.getType() == UIQQFaceCompiler.ElementType.SPAN) {
                UIQQFaceCompiler.ElementList spanElementList = element.getChildList();
            ...
            } else if (element.getType() == UIQQFaceCompiler.ElementType.NEXTLINE) {
                gotoCalNextLine(widthStart, true);
            } else if (element.getType() == UIQQFaceCompiler.ElementType.SPECIAL_BOUNDS_DRAWABLE) {
    ...
    private void measureText(CharSequence text, int widthStart, int widthEnd) {
        ...
        for (int i = 0; i < widths.length; i++) {
            ...
            if (System.currentTimeMillis() - loop_start > 2000) {
                ...
                UILog.d(TAG, "measureText: text = %s, mCurrentCalWidth = %d, " +
    ...
    private UITouchableSpan mCurrentDrawSpan;

    private void drawElements(Canvas canvas, List<UIQQFaceCompiler.Element> elements, int usefulWidth) {
        ...
        for (int i = 0; i < elements.size(); i++) {
            element = elements.get(i);
            UIQQFaceCompiler.ElementType type = element.getType();
            if (type == UIQQFaceCompiler.ElementType.DRAWABLE) {
                onDrawQQFace(canvas, element.getDrawableRes(), null, startLeft, endWidth, i == 0, i == elements.size() - 1);
            } else if (type == UIQQFaceCompiler.ElementType.SPECIAL_BOUNDS_DRAWABLE) {
                onDrawQQFace(canvas, 0, element.getSpecialBoundsDrawable(), startLeft, endWidth, i == 0, i == elements.size() - 1);
            } else if (type == UIQQFaceCompiler.ElementType.TEXT) {
                CharSequence text = element.getText();
            ...
            } else if (type == UIQQFaceCompiler.ElementType.SPAN) {
                UIQQFaceCompiler.ElementList spanElementList = element.getChildList();
            ...
            } else if (type == UIQQFaceCompiler.ElementType.NEXTLINE) {
    ...
    private void toNewDrawLine(int startLeft, boolean paragraph, int usefulWidth) {
        ...
        if (mIsNeedEllipsize) {
            ...
            if (mEllipsize != null && mEllipsize != TextUtils.TruncateAt.END && mCurrentDrawBaseLine > getHeight() - getPaddingBottom()) {
                UILog.d(TAG, "draw outside the visible height, the ellipsize is inaccurate: " +
    ...
    private class SpanInfo {
        ...
        public void onClick() {
            mTouchableSpan.onClick(UIQQFaceView.this);
        }
