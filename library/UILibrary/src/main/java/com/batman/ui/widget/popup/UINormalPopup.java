...
public class UINormalPopup<T extends UIBasePopup> extends UIBasePopup<T> {
    ...
    private int mBorderColorAttr = R.attr.ui_skin_support_popup_border_color;
    ...
    private int mBgColorAttr = R.attr.ui_skin_support_popup_bg;
    ...
    public UINormalPopup(Context context, int width, int height) {
        super(context);
        mInitWidth = width;
        mInitHeight = height;
    }
    ...
    private boolean shouldShowShadow() {
        return mAddShadow && UILayoutHelper.useFeature();
    }
    ...
    private void decorateContentView(ShowInfo showInfo) {
        ...
        if (mBorderColor != NOT_SET) {
            ...
            mBorderUsedColor = UIResHelper.getAttrColor(mContext, mBorderColorAttr);
            builder.border(mBorderColorAttr);
        }
        if (mBgColor != NOT_SET) {
            mBgUsedColor = mBgColor;
        } else if (mBgColorAttr != 0) {
            mBgUsedColor = UIResHelper.getAttrColor(mContext, mBgColorAttr);
            builder.background(mBgColorAttr);
        }

        if (mBorderWidth == NOT_SET) {
            mBorderWidth = UIResHelper.getAttrDimen(mContext, R.attr.ui_popup_border_width);
        }

        UISkinHelper.setSkinValue(contentView, builder);
        builder.release();
        ...
        if (mRadius == NOT_SET) {
            mRadius = UIResHelper.getAttrDimen(mContext, R.attr.ui_popup_radius);
        }
    ...
    private void adjustShowInfo(ShowInfo showInfo) {
        if (shouldShowShadow()) {
            if (mShadowElevation == NOT_SET) {
                mShadowElevation = UIResHelper.getAttrDimen(mContext, R.attr.ui_popup_shadow_elevation);
                mShadowAlpha = UIResHelper.getAttrFloatValue(mContext, R.attr.ui_popup_shadow_alpha);
            }
            if (mShadowInset == NOT_SET) {
                mShadowInset = UIResHelper.getAttrDimen(mContext, R.attr.ui_popup_shadow_inset);
            }
        ...
        if (mShowArrow && showInfo.direction != DIRECTION_CENTER_IN_SCREEN) {
            if (mArrowWidth == NOT_SET) {
                mArrowWidth = UIResHelper.getAttrDimen(mContext, R.attr.ui_popup_arrow_width);
            }
            if (mArrowHeight == NOT_SET) {
                mArrowHeight = UIResHelper.getAttrDimen(mContext, R.attr.ui_popup_arrow_height);
            }
    ...
    private void setAnimationStyle(float anchorProportion, @Direction int direction) {
        ...
        switch (mAnimStyle) {
            case ANIM_GROW_FROM_LEFT:
                mWindow.setAnimationStyle(onTop ? R.style.UI_Animation_PopUpMenu_Left : R.style.UI_Animation_PopDownMenu_Left);
                break;

            case ANIM_GROW_FROM_RIGHT:
                mWindow.setAnimationStyle(onTop ? R.style.UI_Animation_PopUpMenu_Right : R.style.UI_Animation_PopDownMenu_Right);
                break;

            case ANIM_GROW_FROM_CENTER:
                mWindow.setAnimationStyle(onTop ? R.style.UI_Animation_PopUpMenu_Center : R.style.UI_Animation_PopDownMenu_Center);
                break;
            case ANIM_AUTO:
    ...
    class DecorRootView extends FrameLayout implements IUISkinDispatchInterceptor {
        ...
        public boolean intercept(int skinIndex, @NonNull Resources.Theme theme) {
            if (mBorderColor == NOT_SET && mBorderColorAttr != 0) {
                mBorderUsedColor = UIResHelper.getAttrColor(theme, mBorderColorAttr);
            }
            if (mBgColor == NOT_SET && mBgColorAttr != 0) {
                mBgUsedColor = UIResHelper.getAttrColor(theme, mBgColorAttr);
            }
            return false;
        }
