...
public class UISkinRuleBackgroundHandler implements IUISkinRuleHandler {
    ...
    @Override
    public void handle(@NonNull UISkinManager skinManager, @NonNull View view, @NonNull Resources.Theme theme, @NonNull String name, int attr) {
        if(view instanceof UIRoundButton){
            ((UIRoundButton)view).setBgData(
                    UIResHelper.getAttrColorStateList(view.getContext(), theme, attr));
        }else if(view instanceof UIProgressBar){
            view.setBackgroundColor(UIResHelper.getAttrColor(theme, attr));
        }else if(view instanceof UISlider){
            ((UISlider)view).setBarNormalColor(UIResHelper.getAttrColor(theme, attr));
        }else{
            UIViewHelper.setBackgroundKeepingPadding(view,
                    UIResHelper.getAttrDrawable(view.getContext(), theme, attr));
        }
    }
}
