...
public class UIContinuousNestedTopAreaBehavior extends UIViewOffsetBehavior<View> {
    ...
    public UIContinuousNestedTopAreaBehavior(Context context) {
        this(context, null);
    }


    public UIContinuousNestedTopAreaBehavior(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    void scroll(@NonNull CoordinatorLayout parent, @NonNull View child, int dy) {
        ...
        if (child instanceof IUIContinuousNestedTopView) {
            unConsumed = ((IUIContinuousNestedTopView) child).consumeScroll(unConsumed);
        }
    ...
    public void onNestedPreScroll(@NonNull CoordinatorLayout parent, @NonNull View child,
        ...
        if (target == child) {
            ...
            if (dy > 0) {
                ...
                if (target instanceof IUIContinuousNestedBottomView) {
                    int contentHeight = ((IUIContinuousNestedBottomView) target).getContentHeight();
                    int minOffset;
        ...
        if (target == child) {
            ...
            if (dyUnconsumed > 0) {
                ...
                if (bottomView == null || bottomView.getVisibility() == View.GONE) {
                    ...
                    int contentHeight = ((IUIContinuousNestedBottomView) bottomView).getContentHeight();
                    ...
                    if (canContentScroll) {
                        ((IUIContinuousNestedBottomView) bottomView).consumeScroll(dyUnconsumed);
                    }
                }
            }
        } else {
            ...
            if (dyUnconsumed < 0) {
                ...
                if (child instanceof IUIContinuousNestedTopView) {
                    ((IUIContinuousNestedTopView) child).consumeScroll(dyUnconsumed);
                }
            }
        }
    }
    ...
    private View findBottomView(CoordinatorLayout parent) {
        for (int i = 0; i < parent.getChildCount(); i++) {
            ...
            if (child instanceof IUIContinuousNestedBottomView) {
                return child;
            }
        }
        return null;
        ...
        public void run() {
            ...
            if (scroller.computeScrollOffset()) {
                ...
                if (mCurrentParent != null && mCurrentChild != null) {
                    ...
                    if(mCurrentParent instanceof UIContinuousNestedScrollLayout){
                        UIContinuousNestedScrollLayout layout = (UIContinuousNestedScrollLayout) mCurrentParent;
