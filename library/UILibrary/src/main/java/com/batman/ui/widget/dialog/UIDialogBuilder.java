 ...
 * 子类也可以通过 override {@link #onCreateTitle(UIDialog, UIDialogView, Context)} 方法自定义</li>
 * <li>中间部分的内容由各个子类决定, 子类通过 override {@link #onCreateContent(UIDialog, UIDialogView, Context)} 方法自定义。</li>
 * <li>下部分是操作区域, 支持添加操作按钮, 通过 {@link #addAction(int, int, UIDialogAction.ActionListener)} 系列方法添加。
 * 子类也可以通过 override {@link #onCreateOperatorLayout(UIDialog, UIDialogView, Context)} 方法自定义。
...
public abstract class UIDialogBuilder<T extends UIDialogBuilder> {
    ...
    protected UIDialogRootLayout mRootView;
    ...
    private int mActionDividerColorAttr = R.attr.ui_skin_support_dialog_action_divider_color;
    ...
    public UIDialog create(@StyleRes int style) {
        ...
        mRootView = new UIDialogRootLayout(dialogContext, mDialogView, onCreateDialogLayoutParams());
        mRootView.setCheckKeyboardOverlay(mCheckKeyboardOverlay);
        mRootView.setOverlayOccurInMeasureCallback(new UIDialogRootLayout.OverlayOccurInMeasureCallback() {
            @Override
            public void call() {
        ...
        checkAndSetId(titleView, R.id.ui_dialog_title_id);
        checkAndSetId(operatorLayout, R.id.ui_dialog_operator_layout_id);
        checkAndSetId(contentLayout, R.id.ui_dialog_content_id);
    ...
    protected void onAfterCreate(@NonNull UIDialog dialog, @NonNull UIDialogRootLayout rootLayout, @NonNull Context context){
    ...
    protected void configRootLayout(@NonNull UIDialogRootLayout rootLayout){

    }

    protected void skinConfigDialogView(UIDialogView dialogView){
        UISkinValueBuilder valueBuilder = UISkinValueBuilder.acquire();
        valueBuilder.background(R.attr.ui_skin_support_dialog_bg);
        UISkinHelper.setSkinValue(dialogView, valueBuilder);
        UISkinValueBuilder.release(valueBuilder);
    }
    protected void skinConfigTitleView(TextView titleView){
        UISkinValueBuilder valueBuilder = UISkinValueBuilder.acquire();
        valueBuilder.textColor(R.attr.ui_skin_support_dialog_title_text_color);
        UISkinHelper.setSkinValue(titleView, valueBuilder);
        UISkinValueBuilder.release(valueBuilder);
    }
    protected void skinConfigActionContainer(ViewGroup actionContainer){
        UISkinValueBuilder valueBuilder = UISkinValueBuilder.acquire();
        valueBuilder.topSeparator(R.attr.ui_skin_support_dialog_action_container_separator_color);
        UISkinHelper.setSkinValue(actionContainer, valueBuilder);
        UISkinValueBuilder.release(valueBuilder);
    }

    @NonNull
    protected UIDialogView onCreateDialogView(@NonNull Context context){
        UIDialogView dialogView = new UIDialogView(context);
        dialogView.setBackground(UIResHelper.getAttrDrawable(context, R.attr.ui_skin_support_dialog_bg));
        dialogView.setRadius(UIResHelper.getAttrDimen(context, R.attr.ui_dialog_radius));
    ...
    protected View onCreateTitle(@NonNull UIDialog dialog, @NonNull UIDialogView parent, @NonNull Context context) {
        if (hasTitle()) {
            ...
            tv.setId(R.id.ui_dialog_title_id);
            tv.setText(mTitle);
            UIResHelper.assignTextViewWithAttr(tv, R.attr.ui_dialog_title_style);
    ...
    @Nullable
    protected abstract View onCreateContent(@NonNull UIDialog dialog, @NonNull UIDialogView parent, @NonNull Context context);


    protected UIWrapContentScrollView wrapWithScroll(@NonNull View view){
    ...
    @Nullable
    protected View onCreateOperatorLayout(@NonNull final UIDialog dialog, @NonNull UIDialogView parent, @NonNull Context context) {
        ...
        if (size > 0) {
            ...
            final UILinearLayout layout = new UILinearLayout(context, null, R.attr.ui_dialog_action_container_style);
            layout.setId(R.id.ui_dialog_operator_layout_id);
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (mActionDividerThickness > 0 && i > 0 && spaceInsertPos != i) {
                    int color = mActionDividerColorAttr == 0 ? mActionDividerColor :
                            UISkinHelper.getSkinColor(actionView, mActionDividerColorAttr);
                    if (mActionContainerOrientation == VERTICAL) {
