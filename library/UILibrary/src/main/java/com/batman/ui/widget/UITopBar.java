...
import com.batman.ui.skin.IUISkinHandlerView;
 ...
 * </ul>
 */
public class UITopBar extends UIRelativeLayout implements IUISkinHandlerView, IUISkinDefaultAttrProvider {
    ...
    private UIQQFaceView mTitleView; // 显示 title 文字的 TextView
    private UIQQFaceView mSubTitleView; // 显示 subTitle 文字的 TextView
    ...
    static {
        ...
        sDefaultSkinAttrs.put(UISkinValueBuilder.BOTTOM_SEPARATOR, R.attr.ui_skin_support_topbar_separator_color);
        sDefaultSkinAttrs.put(UISkinValueBuilder.BACKGROUND, R.attr.ui_skin_support_topbar_bg);
    }
    ...
    public UITopBar(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    void init(Context context, AttributeSet attrs) {
        init(context, attrs, R.attr.UITopBarStyle);
    }

    void init(Context context, AttributeSet attrs, int defStyleAttr) {
        TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.UITopBar, defStyleAttr, 0);
        mLeftBackDrawableRes = array.getResourceId(R.styleable.UITopBar_ui_topbar_left_back_drawable_id, R.drawable.ui_icon_topbar_back);
        mTitleGravity = array.getInt(R.styleable.UITopBar_ui_topbar_title_gravity, Gravity.CENTER);
        mTitleTextSize = array.getDimensionPixelSize(R.styleable.UITopBar_ui_topbar_title_text_size, UIDisplayHelper.sp2px(context, 17));
        mTitleTextSizeWithSubTitle = array.getDimensionPixelSize(R.styleable.UITopBar_ui_topbar_title_text_size, UIDisplayHelper.sp2px(context, 16));
        mSubTitleTextSize = array.getDimensionPixelSize(R.styleable.UITopBar_ui_topbar_subtitle_text_size, UIDisplayHelper.sp2px(context, 11));
        mTitleTextColor = array.getColor(R.styleable.UITopBar_ui_topbar_title_color, UIResHelper.getAttrColor(context, R.attr.ui_config_color_gray_1));
        mSubTitleTextColor = array.getColor(R.styleable.UITopBar_ui_topbar_subtitle_color, UIResHelper.getAttrColor(context, R.attr.ui_config_color_gray_4));
        mTitleMarginHorWhenNoBtnAside = array.getDimensionPixelSize(R.styleable.UITopBar_ui_topbar_title_margin_horizontal_when_no_btn_aside, 0);
        mTitleContainerPaddingHor = array.getDimensionPixelSize(R.styleable.UITopBar_ui_topbar_title_container_padding_horizontal, 0);
        mTopBarImageBtnWidth = array.getDimensionPixelSize(R.styleable.UITopBar_ui_topbar_image_btn_width, UIDisplayHelper.dp2px(context, 48));
        mTopBarImageBtnHeight = array.getDimensionPixelSize(R.styleable.UITopBar_ui_topbar_image_btn_height, UIDisplayHelper.dp2px(context, 48));
        mTopBarTextBtnPaddingHor = array.getDimensionPixelSize(R.styleable.UITopBar_ui_topbar_text_btn_padding_horizontal, UIDisplayHelper.dp2px(context, 12));
        mTopBarTextBtnTextColor = array.getColorStateList(R.styleable.UITopBar_ui_topbar_text_btn_color_state_list);
        mTopBarTextBtnTextSize = array.getDimensionPixelSize(R.styleable.UITopBar_ui_topbar_text_btn_text_size, UIDisplayHelper.sp2px(context, 16));

        mTitleTypeface = array.getBoolean(R.styleable.UITopBar_ui_topbar_title_bold, false) ? Typeface.DEFAULT_BOLD : null;
        mSubTitleTypeface = array.getBoolean(R.styleable.UITopBar_ui_topbar_subtitle_bold, false) ? Typeface.DEFAULT_BOLD : null;
        mTopBarTextBtnTypeface = array.getBoolean(R.styleable.UITopBar_ui_topbar_text_btn_bold, false) ? Typeface.DEFAULT_BOLD : null;
        int ellipsize = array.getInt(R.styleable.UITopBar_android_ellipsize, -1) ;
        switch (ellipsize) {
            case 1:
                mEllipsize = TruncateAt.START;
                break;
            case 2:
                mEllipsize = TruncateAt.MIDDLE;
                break;
            case 3:
    ...
    protected void onAttachedToWindow() {
        ...
        while (parent instanceof View) {
            if (parent instanceof UICollapsingTopBarLayout) {
    ...
    public UIQQFaceView setTitle(String title) {
        UIQQFaceView titleView = getTitleView();
        titleView.setText(title);
        if (UILangHelper.isNullOrEmpty(title)) {
    ...
    private UIQQFaceView getTitleView() {
        if (mTitleView == null) {
            mTitleView = new UIQQFaceView(getContext());
            ...
            provider.setDefaultSkinAttr(UISkinValueBuilder.TEXT_COLOR, R.attr.ui_skin_support_topbar_title_color);
            mTitleView.setTag(R.id.ui_skin_default_attr_provider, provider);
    ...
    private void updateTitleViewStyle() {
        if (mTitleView != null) {
            if (mSubTitleView == null || UILangHelper.isNullOrEmpty(mSubTitleView.getText())) {
    ...
    public UIQQFaceView setSubTitle(String subTitle) {
        UIQQFaceView subTitleView = getSubTitleView();
        subTitleView.setText(subTitle);
        if (UILangHelper.isNullOrEmpty(subTitle)) {
    ...
    private UIQQFaceView getSubTitleView() {
        if (mSubTitleView == null) {
            mSubTitleView = new UIQQFaceView(getContext());
            ...
            UISkinSimpleDefaultAttrProvider provider = new UISkinSimpleDefaultAttrProvider();
            provider.setDefaultSkinAttr(UISkinValueBuilder.TEXT_COLOR, R.attr.ui_skin_support_topbar_subtitle_color);
            mSubTitleView.setTag(R.id.ui_skin_default_attr_provider, provider);
            LinearLayout.LayoutParams titleLp = generateTitleViewAndSubTitleViewLp();
    ...
    private LayoutParams generateTitleContainerViewLp() {
        return new LayoutParams(LayoutParams.MATCH_PARENT,
                UIResHelper.getAttrDimen(getContext(), R.attr.ui_topbar_height));
    }
     ...
     * @return 返回生成的按钮
     */
    public UIAlphaImageButton addRightImageButton(int drawableResId, boolean followTintColor, int viewId) {
        UIAlphaImageButton rightButton = generateTopBarImageButton(drawableResId, followTintColor);
     ...
     * @return 返回生成的按钮
     */
    public UIAlphaImageButton addLeftImageButton(int drawableResId, boolean followTintColor, int viewId) {
        UIAlphaImageButton leftButton = generateTopBarImageButton(drawableResId, followTintColor);
    ...
    private IUISkinDefaultAttrProvider mTopBarTextDefaultAttrProvider;
    ...
    private Button generateTopBarTextButton(String text) {
        ...
        if (mTopBarTextDefaultAttrProvider == null) {
            ...
            provider.setDefaultSkinAttr(
                    UISkinValueBuilder.TEXT_COLOR, R.attr.ui_skin_support_topbar_text_btn_color_state_list);
            mTopBarTextDefaultAttrProvider = provider;

        }
        button.setTag(R.id.ui_skin_default_attr_provider, mTopBarTextDefaultAttrProvider);
    ...
    private IUISkinDefaultAttrProvider mTopBarImageColorTintColorProvider;
     ...
     */
    private UIAlphaImageButton generateTopBarImageButton(int imageResourceId, boolean followTintColor) {
        UIAlphaImageButton imageButton = new UIAlphaImageButton(getContext());
        if (followTintColor) {
            if (mTopBarImageColorTintColorProvider == null) {
                UISkinSimpleDefaultAttrProvider provider = new UISkinSimpleDefaultAttrProvider();
                provider.setDefaultSkinAttr(
                        UISkinValueBuilder.TINT_COLOR, R.attr.ui_skin_support_topbar_image_tint_color);
                mTopBarImageColorTintColorProvider = provider;
            }
            imageButton.setTag(R.id.ui_skin_default_attr_provider, mTopBarImageColorTintColorProvider);
        }
    ...
    public UIAlphaImageButton addLeftBackImageButton() {
        return addLeftImageButton(mLeftBackDrawableRes, R.id.ui_topbar_item_left_back);
    }
    ...
    int getTopBarHeight() {
        if (mTopBarHeight == -1) {
            mTopBarHeight = UIResHelper.getAttrDimen(getContext(), R.attr.ui_topbar_height);
        }
    ...
    @Override
    public void handle(@NonNull UISkinManager manager, int skinIndex, @NonNull Resources.Theme theme, @Nullable SimpleArrayMap<String, Integer> attrs) {
        if (attrs != null) {
            for (int i = 0; i < attrs.size(); i++) {
                ...
                if (getParent() instanceof UITopBarLayout &&
                        (UISkinValueBuilder.BACKGROUND.equals(key) ||
                                UISkinValueBuilder.BOTTOM_SEPARATOR.equals(key))) {
                    // handled by parent
                    continue;
                }
