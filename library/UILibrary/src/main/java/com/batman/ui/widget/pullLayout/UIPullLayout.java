...
public class UIPullLayout extends FrameLayout implements NestedScrollingParent2 {
    ...
    private UIViewOffsetHelper mTargetOffsetHelper;
    ...
    public UIPullLayout(@NonNull Context context) {
        this(context, null);
    }

    public UIPullLayout(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, R.attr.UIPullLayoutStyle);
    }

    public UIPullLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray array = context.obtainStyledAttributes(attrs,
                R.styleable.UIPullLayout, defStyleAttr, 0);
        mEnabledEdges = array.getInt(R.styleable.UIPullLayout_ui_pull_enable_edge, PUL_EDGE_ALL);
        array.recycle();
            ...
            if (lp.isTarget) {
                if (isTargetSet) {
                    throw new RuntimeException(
                            "More than one view in xml are marked by ui_is_target = true.");
                }
                isTargetSet = true;
                setTargetView(view);
            } else {
                if ((edgesSet & lp.edge) != 0) {
                    ...
                    throw new RuntimeException("More than one view in xml marked by ui_layout_edge = " + text);
                }
                edgesSet |= lp.edge;
    ...
    private void innerSetTargetView(@NonNull View view) {
        mTargetView = view;
        mTargetOffsetHelper = new UIViewOffsetHelper(view);
    }
...
//    }
    ...
    public static class LayoutParams extends FrameLayout.LayoutParams {
        ...
        public LayoutParams(Context c, AttributeSet attrs) {
            ...
            final TypedArray a = c.obtainStyledAttributes(attrs,
                    R.styleable.UIPullLayout_Layout);
            isTarget = a.getBoolean(R.styleable.UIPullLayout_Layout_ui_is_target, false);
            if (!isTarget) {
                edge = a.getInteger(R.styleable.UIPullLayout_Layout_ui_pull_edge, PULL_EDGE_TOP);
                try {
                    targetTriggerOffset = a.getDimensionPixelSize(
                            R.styleable.UIPullLayout_Layout_ui_target_view_trigger_offset,
                            ViewGroup.LayoutParams.WRAP_CONTENT);
                } catch (Exception ignore) {
                    int intValue = a.getInt(R.styleable.UIPullLayout_Layout_ui_target_view_trigger_offset, ViewGroup.LayoutParams.WRAP_CONTENT);
                ...
                canOverPull = a.getBoolean(
                        R.styleable.UIPullLayout_Layout_ui_can_over_pull, false);
                pullRate = a.getFloat(
                        R.styleable.UIPullLayout_Layout_ui_pull_rate, pullRate);
                needReceiveFlingFromTarget = a.getBoolean(
                        R.styleable.UIPullLayout_Layout_ui_need_receive_fling_from_target_view, true);
                receivedFlingFraction = a.getFloat(
                        R.styleable.UIPullLayout_Layout_ui_received_fling_fraction, receivedFlingFraction);
                actionInitOffset = a.getDimensionPixelSize(R.styleable.UIPullLayout_Layout_ui_action_view_init_offset, 0);
                scrollSpeedPerPixel = a.getFloat(R.styleable.UIPullLayout_Layout_ui_scroll_speed_per_pixel, scrollSpeedPerPixel);
                triggerUntilScrollToTriggerOffset = a.getBoolean(R.styleable.UIPullLayout_Layout_ui_trigger_until_scroll_to_trigger_offset, false);
                scrollToTriggerOffsetAfterTouchUp = a.getBoolean(R.styleable.UIPullLayout_Layout_ui_scroll_to_trigger_offset_after_touch_up, true);
            }
            a.recycle();
        }
    ...
    public final static class PullAction {
        ...
        private final UIViewOffsetHelper mViewOffsetHelper;
        ...
        PullAction(@NonNull View actionView,
            ...
            mViewOffsetHelper = new UIViewOffsetHelper(actionView);
    ...
    public static class PullActionBuilder {
        ...
        PullAction build() {
            if (mActionViewOffsetCalculator == null) {
                mActionViewOffsetCalculator = new UIAlwaysFollowOffsetCalculator();
            }
