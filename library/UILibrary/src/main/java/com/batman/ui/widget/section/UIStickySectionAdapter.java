...
import static com.batman.ui.widget.section.UISection.ITEM_INDEX_CUSTOM_OFFSET;
import static com.batman.ui.widget.section.UISection.ITEM_INDEX_LOAD_AFTER;
import static com.batman.ui.widget.section.UISection.ITEM_INDEX_LOAD_BEFORE;
import static com.batman.ui.widget.section.UISection.ITEM_INDEX_SECTION_HEADER;

public abstract class UIStickySectionAdapter<
        H extends UISection.Model<H>, T extends UISection.Model<T>, VH extends UIStickySectionAdapter.ViewHolder> extends RecyclerView.Adapter<VH> {
    ...
    private List<UISection<H, T>> mBackupData = new ArrayList<>();
    private List<UISection<H, T>> mCurrentData = new ArrayList<>();

    private SparseIntArray mSectionIndex = new SparseIntArray();
    private SparseIntArray mItemIndex = new SparseIntArray();
    private ArrayList<UISection<H, T>> mLoadingBeforeSections = new ArrayList<>(2);
    private ArrayList<UISection<H, T>> mLoadingAfterSections = new ArrayList<>(2);
     ...
     */
    public final void setData(@Nullable List<UISection<H, T>> data) {
        setData(data, true);
    }
     ...
     */
    public final void setData(@Nullable List<UISection<H, T>> data, boolean onlyMutateState){
     ...
     */
    public final void setData(@Nullable List<UISection<H, T>> data, boolean onlyMutateState, boolean checkLock) {
     ...
     */
    protected void beforeDiffInSet(List<UISection<H, T>> oldData, List<UISection<H, T>> newData) {
     ...
     * @param onlyMutateState   this is used to backup for next diff. True to use shallow copy, false tp use deep copy.
     */
    public final void setDataWithoutDiff(@Nullable List<UISection<H, T>> data, boolean onlyMutateState){
     ...
     * @param checkLock       check section lock
     */
    public final void setDataWithoutDiff(@Nullable List<UISection<H, T>> data, boolean onlyMutateState, boolean checkLock) {
        mLoadingBeforeSections.clear();
        mLoadingAfterSections.clear();
        mCurrentData.clear();
        if (data != null) {
        ...
        UISectionDiffCallback callback = createDiffCallback(mBackupData, mCurrentData);
        callback.cloneNewIndexTo(mSectionIndex, mItemIndex);
        notifyDataSetChanged();
        mBackupData.clear();
        for (UISection<H, T> section : mCurrentData) {
    ...
    protected UISectionDiffCallback<H, T> createDiffCallback(
            List<UISection<H, T>> lastData, List<UISection<H, T>> currentData) {
        return new UISectionDiffCallback<>(lastData, currentData);
    }
    ...
    public int getItemIndex(int position) {
        if (position < 0 || position >= mItemIndex.size()) {
            return UISection.ITEM_INDEX_UNKNOWN;
        }
    ...
    public int getSectionIndex(int position) {
        if (position < 0 || position >= mSectionIndex.size()) {
            return UISection.SECTION_INDEX_UNKNOWN;
        }
        return mSectionIndex.get(position);
    }

    @Nullable
    public UISection<H, T> getSection(int position) {
        if (position < 0 || position >= mSectionIndex.size()) {
            return null;
        }
    ...
    @Nullable
    public UISection<H, T> getSectionDirectly(int index) {
    ...
    public boolean isSectionFold(int position) {
        UISection<H, T> section = getSection(position);
        if (section == null) {
            return false;
        }
    ...
    public void finishLoadMore(UISection<H, T> section, List<T> itemList,
     ...
     * @param section
     */
    private void lock(UISection<H, T> section) {
    ...
    private void lockBefore(int current, boolean needLock) {
        while (current >= 0) {
            UISection<H, T> section = mCurrentData.get(current);
            if (needLock) {
     ...
     */
    public void scrollToSectionHeader(@NonNull UISection<H, T> targetSection, boolean scrollToTop) {
        if (mViewCallback == null) {
            return;
        }
        for (int i = 0; i < mCurrentData.size(); i++) {
            UISection<H, T> section = mCurrentData.get(i);
    ...
    private void safeScrollToSection(@NonNull UISection<H, T> targetSection, boolean scrollToTop) {
        for (int i = 0; i < mSectionIndex.size(); i++) {
            ...
            if (itemIndex == ITEM_INDEX_SECTION_HEADER) {
                UISection<H, T> temp = mCurrentData.get(sectionIndex);
     ...
     * @param scrollToTop   True to scroll to recyclerView Top, false to scroll to visible area.
     */
    public void scrollToSectionItem(@Nullable UISection<H, T> targetSection, @NonNull T targetItem, boolean scrollToTop) {
        if (mViewCallback == null) {
            return;
        }
    ...
    private void safeScrollToSectionItem(@NonNull UISection<H, T> targetSection, @NonNull T item, boolean scrollToTop) {
        for (int i = 0; i < mItemIndex.size(); i++) {
            int position = mItemIndex.keyAt(i);
            int itemIndex = mItemIndex.valueAt(i);
            if (itemIndex < 0) {
    ...
    public int findCustomPosition(int sectionIndex, int customItemIndex, boolean unFoldTargetSection) {
        int itemIndex = UISection.ITEM_INDEX_CUSTOM_OFFSET + customItemIndex;
    ...
    public int findPosition(int sectionIndex, int itemIndex, boolean unFoldTargetSection) {
        if (unFoldTargetSection && sectionIndex >= 0) {
            UISection<H, T> section = mCurrentData.get(sectionIndex);
    ...
    public int findPosition(PositionFinder<H, T> positionFinder, boolean unFoldTargetSection) {
        if (!unFoldTargetSection) {
            for (int i = 0; i < getItemCount(); i++) {
                UISection<H, T> section = getSection(i);
                if (section == null) {
                    continue;
                }
        ...
        UISection<H, T> targetSection = null;
        T targetItem = null;
        loop:
        for (int i = 0; i < mCurrentData.size(); i++) {
            UISection<H, T> section = mCurrentData.get(i);
    ...
    public final void onBindViewHolder(@NonNull final VH vh, int position) {
        ...
        if (itemIndex == ITEM_INDEX_SECTION_HEADER) {
            ...
            onBindCustomItem(vh, position, section, itemIndex - UISection.ITEM_INDEX_CUSTOM_OFFSET);
        }
    ...
    protected void onBindSectionHeader(VH holder, int position, UISection<H, T> section) {

    }

    protected void onBindSectionItem(VH holder, int position, UISection<H, T> section, int itemIndex) {

    }

    protected void onBindSectionLoadingItem(VH holder, int position, UISection<H, T> section, boolean loadingBefore) {

    }

    protected void onBindCustomItem(VH holder, int position, @Nullable UISection<H, T> section, int itemIndex) {
    ...
    public final int getItemViewType(int position) {
        int itemIndex = getItemIndex(position);
        if (itemIndex == UISection.ITEM_INDEX_UNKNOWN) {
            // UIStickySectionItemDecoration uses findFirstVisibleItemPosition to get the layout position
            // it may be exceed the adapter position range if layout is not updated in time
            Log.e(TAG, "the item index is undefined, you may need to check your data if not called by UIStickySectionItemDecoration.");
    ...
    public void onViewAttachedToWindow(@NonNull VH holder) {
        if (holder.getItemViewType() == ITEM_TYPE_SECTION_LOADING && mCallback != null) {
            if (!holder.isLoadError) {
                UISection<H, T> section = getSection(holder.getAdapterPosition());
                if (section != null) {
    ...
    public interface Callback<H extends UISection.Model<H>, T extends UISection.Model<T>> {
        void loadMore(UISection<H, T> section, boolean loadMoreBefore);
    ...
    public interface PositionFinder<H extends UISection.Model<H>, T extends UISection.Model<T>> {
        /**
         ...
         * @return
         */
        boolean find(@NonNull UISection<H, T> section, @Nullable T item);
    }
