...
import com.batman.ui.skin.IUISkinDispatchInterceptor;
import com.batman.ui.skin.UISkinHelper;
import com.batman.ui.util.UICollapsingTextHelper;
...
public class UICollapsingTopBarLayout extends FrameLayout implements IWindowInsetLayout, IUISkinDispatchInterceptor {
    ...
    final UICollapsingTextHelper mCollapsingTextHelper;
    ...
    public UICollapsingTopBarLayout(Context context) {
        this(context, null);
    }

    public UICollapsingTopBarLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public UICollapsingTopBarLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        mCollapsingTextHelper = new UICollapsingTextHelper(this);
        mCollapsingTextHelper.setTextSizeInterpolator(UIInterpolatorStaticHolder.DECELERATE_INTERPOLATOR);

        UIViewHelper.checkAppCompatTheme(context);

        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.UICollapsingTopBarLayout, defStyleAttr, 0);

        mCollapsingTextHelper.setExpandedTextGravity(
                a.getInt(R.styleable.UICollapsingTopBarLayout_ui_expandedTitleGravity,
                        Gravity.CENTER_HORIZONTAL | Gravity.BOTTOM));
        mCollapsingTextHelper.setCollapsedTextGravity(
                a.getInt(R.styleable.UICollapsingTopBarLayout_ui_collapsedTitleGravity,
                        GravityCompat.START | Gravity.CENTER_VERTICAL));


        mExpandedMarginStart = mExpandedMarginTop = mExpandedMarginEnd = mExpandedMarginBottom =
                a.getDimensionPixelSize(R.styleable.UICollapsingTopBarLayout_ui_expandedTitleMargin, 0);

        if (a.hasValue(R.styleable.UICollapsingTopBarLayout_ui_expandedTitleMarginStart)) {
            mExpandedMarginStart = a.getDimensionPixelSize(
                    R.styleable.UICollapsingTopBarLayout_ui_expandedTitleMarginStart, 0);
        }
        if (a.hasValue(R.styleable.UICollapsingTopBarLayout_ui_expandedTitleMarginEnd)) {
            mExpandedMarginEnd = a.getDimensionPixelSize(
                    R.styleable.UICollapsingTopBarLayout_ui_expandedTitleMarginEnd, 0);
        }
        if (a.hasValue(R.styleable.UICollapsingTopBarLayout_ui_expandedTitleMarginTop)) {
            mExpandedMarginTop = a.getDimensionPixelSize(
                    R.styleable.UICollapsingTopBarLayout_ui_expandedTitleMarginTop, 0);
        }
        if (a.hasValue(R.styleable.UICollapsingTopBarLayout_ui_expandedTitleMarginBottom)) {
            mExpandedMarginBottom = a.getDimensionPixelSize(
                    R.styleable.UICollapsingTopBarLayout_ui_expandedTitleMarginBottom, 0);
        }

        mCollapsingTitleEnabled = a.getBoolean(R.styleable.UICollapsingTopBarLayout_ui_titleEnabled, true);
        setTitle(a.getText(R.styleable.UICollapsingTopBarLayout_ui_title));

        // First load the default text appearances
        mCollapsingTextHelper.setExpandedTextAppearance(R.style.UI_CollapsingTopBarLayoutExpanded);
        mCollapsingTextHelper.setCollapsedTextAppearance(R.style.UI_CollapsingTopBarLayoutCollapsed);

        // Now overlay any custom text appearances
        if (a.hasValue(R.styleable.UICollapsingTopBarLayout_ui_expandedTitleTextAppearance)) {
            mCollapsingTextHelper.setExpandedTextAppearance(
                    a.getResourceId(R.styleable.UICollapsingTopBarLayout_ui_expandedTitleTextAppearance, 0));
        }
        if (a.hasValue(R.styleable.UICollapsingTopBarLayout_ui_collapsedTitleTextAppearance)) {
            mCollapsingTextHelper.setCollapsedTextAppearance(
                    a.getResourceId(R.styleable.UICollapsingTopBarLayout_ui_collapsedTitleTextAppearance, 0));
        }

        mScrimVisibleHeightTrigger = a.getDimensionPixelSize(
                R.styleable.UICollapsingTopBarLayout_ui_scrimVisibleHeightTrigger, -1);

        mScrimAnimationDuration = a.getInt(
                R.styleable.UICollapsingTopBarLayout_ui_scrimAnimationDuration,
                DEFAULT_SCRIM_ANIMATION_DURATION);


        mTopBarId = a.getResourceId(R.styleable.UICollapsingTopBarLayout_ui_topBarId, -1);

        if (a.getBoolean(R.styleable.UICollapsingTopBarLayout_ui_followTopBarCommonSkin, false)) {
            followTopBarCommonSkin();
        } else {
            setContentScrimInner(a.getDrawable(R.styleable.UICollapsingTopBarLayout_ui_contentScrim));
            setStatusBarScrimInner(a.getDrawable(R.styleable.UICollapsingTopBarLayout_ui_statusBarScrim));
        }
        a.recycle();
    ...
    public void followTopBarCommonSkin() {
        setCollapsedTextColorSkinAttr(R.attr.ui_skin_support_topbar_title_color);
        setExpandedTextColorSkinAttr(R.attr.ui_skin_support_topbar_title_color);
        setContentScrimSkinAttr(R.attr.ui_skin_support_topbar_bg);
        setStatusBarScrimSkinAttr(R.attr.ui_skin_support_topbar_bg);
    }

    @Override
    public void onViewAdded(View child) {
        super.onViewAdded(child);
        if (child instanceof UITopBar) {
            ((UITopBar) child).disableBackgroundSetter();
        }
    }
    ...
    private void ensureToolbar() {
        ...
        if (mTopBarId != -1) {
            ...
            mTopBar = (UITopBar) findViewById(mTopBarId);
            if (mTopBar != null) {
        ...
        if (mTopBar == null) {
            ...
            UITopBar topBar = null;
            for (int i = 0, count = getChildCount(); i < count; i++) {
                final View child = getChildAt(i);
                if (child instanceof UITopBar) {
                    topBar = (UITopBar) child;
                    break;
                }
            }
            mTopBar = topBar;
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (mCollapsingTitleEnabled) {
            ...
            UIViewHelper.getDescendantRect(this, mTopBar, mTmpRect);
//            mTmpRect.top = mTmpRect.top - topBarInsetAdjustTop;
    ...
    static UIViewOffsetHelper getViewOffsetHelper(View view) {
        ...
        if (mScrimAnimator == null) {
            ...
            mScrimAnimator.setInterpolator(
                    targetAlpha > mScrimAlpha
                            ? UIInterpolatorStaticHolder.FAST_OUT_LINEAR_IN_INTERPOLATOR
                            : UIInterpolatorStaticHolder.LINEAR_OUT_SLOW_IN_INTERPOLATOR);
    ...
    void setScrimAlpha(int alpha) {
        if (alpha != mScrimAlpha) {
            ...
            mScrimAlpha = alpha;
            ViewCompat.postInvalidateOnAnimation(UICollapsingTopBarLayout.this);
        }
    }
    ...
    public void setContentScrimSkinAttr(int contentScrimSkinAttr) {
        ...
        if (contentScrimSkinAttr != 0) {
            setStatusBarScrimInner(UISkinHelper.getSkinDrawable(this, contentScrimSkinAttr));
        }
    }
    ...
    public void setStatusBarScrimSkinAttr(int statusBarScrimSkinAttr) {
        ...
        if (mStatusBarScrimSkinAttr != 0) {
            setStatusBarScrimInner(UISkinHelper.getSkinDrawable(this, statusBarScrimSkinAttr));
        }
    }
    ...
    public void setCollapsedTextColorSkinAttr(int attr) {
        ...
        if (attr != 0) {
            mCollapsingTextHelper.setCollapsedTextColor(
                    UISkinHelper.getSkinColorStateList(this, attr));
        }
    }
    ...
    public boolean applySystemWindowInsets19(Rect insets) {
        ...
        if (!UILangHelper.objectEquals(mLastInsets, newInsets)) {
    ...
    public static class LayoutParams extends FrameLayout.LayoutParams {
        ...
        public LayoutParams(Context c, AttributeSet attrs) {
            ...
            TypedArray a = c.obtainStyledAttributes(attrs,
                    R.styleable.UICollapsingTopBarLayout_Layout);
            mCollapseMode = a.getInt(
                    R.styleable.UICollapsingTopBarLayout_Layout_ui_layout_collapseMode,
                    COLLAPSE_MODE_OFF);
            setParallaxMultiplier(a.getFloat(
                    R.styleable.UICollapsingTopBarLayout_Layout_ui_layout_collapseParallaxMultiplier,
    ...
    final int getMaxOffsetForPinChild(View child) {
        final UIViewOffsetHelper offsetHelper = getViewOffsetHelper(child);
    ...
    private class OffsetUpdateListener implements AppBarLayout.OnOffsetChangedListener {
        ...
        public void onOffsetChanged(AppBarLayout layout, int verticalOffset) {
                ...
                switch (lp.mCollapseMode) {
                    case LayoutParams.COLLAPSE_MODE_PIN:
                        offsetHelper.setTopAndBottomOffset(
                                UILangHelper.constrain(-verticalOffset, 0, getMaxOffsetForPinChild(child)));
                        break;
            ...
            final int expandRange = getHeight() - ViewCompat.getMinimumHeight(
                    UICollapsingTopBarLayout.this) - insetTop;
            ...
            for (OnOffsetUpdateListener listener : mOnOffsetUpdateListeners) {
                listener.onOffsetChanged(
                        UICollapsingTopBarLayout.this, verticalOffset, expansionFraction);
            }
        }
    }
    ...
    public boolean intercept(int skinIndex, @NonNull Resources.Theme theme) {
        if (mContentScrimSkinAttr != 0) {
            setContentScrimInner(UIResHelper.getAttrDrawable(getContext(), theme, mContentScrimSkinAttr));
        }
        if (mStatusBarScrimSkinAttr != 0) {
            setStatusBarScrimInner(UIResHelper.getAttrDrawable(getContext(), theme, mStatusBarScrimSkinAttr));
        }

        if (mCollapsedTextColorSkinAttr != 0) {
            mCollapsingTextHelper.setCollapsedTextColor(
                    UISkinHelper.getSkinColorStateList(this, mCollapsedTextColorSkinAttr));
        }
        if (mExpandedTextColorSkinAttr != 0) {
            mCollapsingTextHelper.setExpandedTextColor(
                    UISkinHelper.getSkinColorStateList(this, mExpandedTextColorSkinAttr)
            );
        }
        return false;
    }
    ...
    public interface OnOffsetUpdateListener {
        void onOffsetChanged(UICollapsingTopBarLayout layout, int offset, float expandFraction);
    }
}
