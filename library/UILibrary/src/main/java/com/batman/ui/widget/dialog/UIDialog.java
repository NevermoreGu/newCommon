...
/**
 * UIDialog 对话框一般由 {@link UIDialogBuilder} 及其子类创建, 不同的 Builder 可以创建不同类型的对话框,
 * 例如消息类型的对话框、菜单项对话框等等。
 *
 * @author cginechen
 * @date 2015-10-20
...
public class UIDialog extends UIBaseDialog {
    ...
    public UIDialog(Context context, int styleRes) {
    ...
    public static class MessageDialogBuilder extends UIDialogBuilder<MessageDialogBuilder> {
        ...
        @Nullable
        @Override
        protected View onCreateContent(@NonNull UIDialog dialog, @NonNull UIDialogView parent, @NonNull Context context) {
            if (mMessage != null && mMessage.length() != 0) {
                UISpanTouchFixTextView tv = new UISpanTouchFixTextView(context);
                assignMessageTvWithAttr(tv, hasTitle(), R.attr.ui_dialog_message_content_style);
                tv.setText(mMessage);
                tv.setMovementMethodDefault();

                UISkinValueBuilder valueBuilder = UISkinValueBuilder.acquire();
                valueBuilder.textColor(R.attr.ui_skin_support_dialog_message_text_color);
                UISkinHelper.setSkinValue(tv, valueBuilder);
                UISkinValueBuilder.release(valueBuilder);
        ...
        @Nullable
        @Override
        protected View onCreateTitle(@NonNull UIDialog dialog, @NonNull UIDialogView parent, @NonNull Context context) {
        ...
        public static void assignMessageTvWithAttr(TextView messageTv, boolean hasTitle, int defAttr) {
            UIResHelper.assignTextViewWithAttr(messageTv, defAttr);

            if (!hasTitle) {
                TypedArray a = messageTv.getContext().obtainStyledAttributes(null,
                        R.styleable.UIDialogMessageTvCustomDef, defAttr, 0);
                int count = a.getIndexCount();
                for (int i = 0; i < count; i++) {
                    int attr = a.getIndex(i);
    ...
    public static class CheckBoxMessageDialogBuilder extends UIDialogBuilder<CheckBoxMessageDialogBuilder> {
        ...
        protected View onCreateContent(UIDialog dialog, UIDialogView parent, Context context) {
            if (mMessage != null && mMessage.length() != 0) {
                ...
                MessageDialogBuilder.assignMessageTvWithAttr(mTextView, hasTitle(), R.attr.ui_dialog_message_content_style);
                mTextView.setText(mMessage);
                Drawable drawable = UISkinHelper.getSkinDrawable(mTextView, R.attr.ui_skin_support_s_dialog_check_drawable);
                if (drawable != null) {
                ...
                UISkinValueBuilder valueBuilder = UISkinValueBuilder.acquire();
                valueBuilder.textColor(R.attr.ui_skin_support_dialog_message_text_color);
                valueBuilder.textCompoundLeftSrc(R.attr.ui_skin_support_s_dialog_check_drawable);
                UISkinHelper.setSkinValue(mTextView, valueBuilder);
                UISkinValueBuilder.release(valueBuilder);
    ...
    public static class EditTextDialogBuilder extends UIDialogBuilder<EditTextDialogBuilder> {
        ...
        protected ConstraintLayout.LayoutParams onCreateContentLayoutParams(Context context) {
            ...
            int marginHor = UIResHelper.getAttrDimen(context, R.attr.ui_dialog_padding_horizontal);
            lp.leftMargin = marginHor;
            lp.rightMargin = marginHor;
            lp.topMargin = UIResHelper.getAttrDimen(context, R.attr.ui_dialog_edit_margin_top);
            lp.bottomMargin = UIResHelper.getAttrDimen(context, R.attr.ui_dialog_edit_margin_bottom);
            return lp;
        }
        ...
        @Override
        protected View onCreateContent(UIDialog dialog, UIDialogView parent, Context context) {
            UIConstraintLayout boxLayout = new UIConstraintLayout(context);
            boxLayout.onlyShowBottomDivider(0, 0,
                    UIResHelper.getAttrDimen(context,
                            R.attr.ui_dialog_edit_bottom_line_height),
                    UIResHelper.getAttrColor(context,
                            R.attr.ui_skin_support_dialog_edit_bottom_line_color));
            UISkinValueBuilder builder = UISkinValueBuilder.acquire();
            builder.bottomSeparator(R.attr.ui_skin_support_dialog_edit_bottom_line_color);
            UISkinHelper.setSkinValue(boxLayout, builder);

            mEditText = new AppCompatEditText(context);
            mEditText.setBackgroundResource(0);
            MessageDialogBuilder.assignMessageTvWithAttr(mEditText, hasTitle(), R.attr.ui_dialog_edit_content_style);
            mEditText.setFocusable(true);
            mEditText.setFocusableInTouchMode(true);
            mEditText.setImeOptions(EditorInfo.IME_ACTION_GO);
            mEditText.setId(R.id.ui_dialog_edit_input);

            if (!UILangHelper.isNullOrEmpty(mDefaultText)) {
            ...
            builder.clear();
            builder.textColor(R.attr.ui_skin_support_dialog_edit_text_color);
            builder.hintColor(R.attr.ui_skin_support_dialog_edit_text_hint_color);
            UISkinHelper.setSkinValue(mEditText, builder);
            UISkinValueBuilder.release(builder);


            mRightImageView = new AppCompatImageView(context);
            mRightImageView.setId(R.id.ui_dialog_edit_right_icon);
        ...
        protected ConstraintLayout.LayoutParams createEditTextLayoutParams(Context context) {
            ...
            editLp.rightToLeft = R.id.ui_dialog_edit_right_icon;
            editLp.rightToRight = UIDisplayHelper.dp2px(context, 5);
        ...
        protected ConstraintLayout.LayoutParams createRightIconLayoutParams(Context context) {
            ...
            rightIconLp.bottomToBottom = R.id.ui_dialog_edit_input;
            return rightIconLp;
        }

        @Override
        protected void onAfterCreate(UIDialog dialog, UIDialogRootLayout rootLayout, Context context) {
    ...
    public static class MenuBaseDialogBuilder<T extends UIDialogBuilder> extends UIDialogBuilder<T> {
        ...
        protected ArrayList<UIDialogMenuItemView> mMenuItemViews = new ArrayList<>();
        ...
        @Deprecated
        public T addItem(final UIDialogMenuItemView itemView, final OnClickListener listener) {
            itemView.setMenuIndex(mMenuItemViewsFactoryList.size());
            itemView.setListener(new UIDialogMenuItemView.MenuItemViewListener() {
                @Override
        ...
        public T addItem(final ItemViewFactory itemViewFactory, final OnClickListener listener) {
            mMenuItemViewsFactoryList.add(new ItemViewFactory() {
                @Override
                public UIDialogMenuItemView createItemView(Context context) {
                    UIDialogMenuItemView itemView = itemViewFactory.createItemView(context);
        ...
        @Override
        protected View onCreateContent(UIDialog dialog, UIDialogView parent, Context context) {
            LinearLayout layout = new UILinearLayout(context);
            layout.setOrientation(LinearLayout.VERTICAL);


            TypedArray a = context.obtainStyledAttributes(
                    null, R.styleable.UIDialogMenuContainerStyleDef, R.attr.ui_dialog_menu_container_style, 0);
            int count = a.getIndexCount();
            int paddingTop = 0, paddingBottom = 0, paddingVerWhenSingle = 0,
                    paddingTopWhenTitle = 0, paddingBottomWhenAction = 0, itemHeight = -1;
            for (int i = 0; i < count; i++) {
                int attr = a.getIndex(i);
                if (attr == R.styleable.UIDialogMenuContainerStyleDef_android_paddingTop) {
                    paddingTop = a.getDimensionPixelSize(attr, paddingTop);
                } else if (attr == R.styleable.UIDialogMenuContainerStyleDef_android_paddingBottom) {
                    paddingBottom = a.getDimensionPixelSize(attr, paddingBottom);
                } else if (attr == R.styleable.UIDialogMenuContainerStyleDef_ui_dialog_menu_container_single_padding_vertical) {
                    paddingVerWhenSingle = a.getDimensionPixelSize(attr, paddingVerWhenSingle);
                } else if (attr == R.styleable.UIDialogMenuContainerStyleDef_ui_dialog_menu_container_padding_top_when_title_exist) {
                    paddingTopWhenTitle = a.getDimensionPixelSize(attr, paddingTopWhenTitle);
                } else if (attr == R.styleable.UIDialogMenuContainerStyleDef_ui_dialog_menu_container_padding_bottom_when_action_exist) {
                    paddingBottomWhenAction = a.getDimensionPixelSize(attr, paddingBottomWhenAction);
                } else if (attr == R.styleable.UIDialogMenuContainerStyleDef_ui_dialog_menu_item_height) {
            ...
            for (ItemViewFactory factory : mMenuItemViewsFactoryList) {
                UIDialogMenuItemView itemView = factory.createItemView(context);
    ...
    public static class MenuDialogBuilder extends MenuBaseDialogBuilder<MenuDialogBuilder> {
        ...
        public MenuDialogBuilder addItem(final CharSequence item, OnClickListener listener) {
            addItem(new ItemViewFactory() {
                @Override
                public UIDialogMenuItemView createItemView(Context context) {
                    return new UIDialogMenuItemView.TextItemView(context, item);
                }
            }, listener);
            return this;
        }
    ...
    public static class CheckableDialogBuilder extends MenuBaseDialogBuilder<CheckableDialogBuilder> {
        ...
        protected void onItemClick(int index) {
            for (int i = 0; i < mMenuItemViews.size(); i++) {
                UIDialogMenuItemView itemView = mMenuItemViews.get(i);
                if (i == index) {
        ...
        public CheckableDialogBuilder addItems(CharSequence[] items, OnClickListener listener) {
            for (final CharSequence item : items) {
                addItem(new ItemViewFactory() {
                    @Override
                    public UIDialogMenuItemView createItemView(Context context) {
                        return new UIDialogMenuItemView.MarkItemView(context, item);
                    }
                }, listener);
            }
            return this;
    ...
    public static class MultiCheckableDialogBuilder extends MenuBaseDialogBuilder<MultiCheckableDialogBuilder> {
        ...
        public MultiCheckableDialogBuilder addItems(CharSequence[] items, OnClickListener listener) {
            for (final CharSequence item : items) {
                addItem(new ItemViewFactory() {
                    @Override
                    public UIDialogMenuItemView createItemView(Context context) {
                        return new UIDialogMenuItemView.CheckItemView(context, true, item);
                    }
                }, listener);
            }
            return this;
        ...
        @Override
        protected void onItemClick(int index) {
            UIDialogMenuItemView itemView = mMenuItemViews.get(index);
    ...
    public static abstract class AutoResizeDialogBuilder extends UIDialogBuilder {
        ...
        public abstract View onBuildContent(@NonNull UIDialog dialog, @NonNull Context context);
    }
}
