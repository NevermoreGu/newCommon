...
public class UISkinLayoutInflaterFactory implements LayoutInflater.Factory2 {
    ...
    public UISkinLayoutInflaterFactory(Activity activity, LayoutInflater originLayoutInflater) {
    ...
    public UISkinLayoutInflaterFactory cloneForLayoutInflaterIfNeeded(LayoutInflater layoutInflater){
        ...
        return new UISkinLayoutInflaterFactory(mActivityWeakReference.get(), layoutInflater);
    }
    ...
    public View onCreateView(View parent, String name, Context context, AttributeSet attrs) {
        ...
        if(view == null){
            try{
                ...
                UILog.e(TAG, "Failed to inflate view " + name + "; error: " + e.getMessage());
            }
        }
        ...
        if (view != null) {
            ...
            if (!builder.isEmpty()) {
                UISkinHelper.setSkinValue(view, builder);
            }
    ...
    public void getSkinValueFromAttributeSet(Context context, @Nullable AttributeSet attrs, UISkinValueBuilder builder) {
        ...
        TypedArray a = mEmptyTheme.obtainStyledAttributes(attrs, R.styleable.UISkinDef, 0, 0);
        int count = a.getIndexCount();
        for (int i = 0; i < count; i++) {
            int attr = a.getIndex(i);
            String name = a.getString(attr);
            if (UILangHelper.isNullOrEmpty(name)) {
                continue;
            }
            if (name.startsWith("?")) {
            ...
            if (attr == R.styleable.UISkinDef_ui_skin_background) {
                builder.background(id);
            } else if (attr == R.styleable.UISkinDef_ui_skin_alpha) {
                builder.alpha(id);
            } else if (attr == R.styleable.UISkinDef_ui_skin_border) {
                builder.border(id);
            } else if (attr == R.styleable.UISkinDef_ui_skin_text_color) {
                builder.textColor(id);
            } else if (attr == R.styleable.UISkinDef_ui_skin_second_text_color) {
                builder.secondTextColor(id);
            } else if (attr == R.styleable.UISkinDef_ui_skin_src) {
                builder.src(id);
            } else if (attr == R.styleable.UISkinDef_ui_skin_tint_color) {
                builder.tintColor(id);
            } else if (attr == R.styleable.UISkinDef_ui_skin_separator_top) {
                builder.topSeparator(id);
            } else if (attr == R.styleable.UISkinDef_ui_skin_separator_right) {
                builder.rightSeparator(id);
            } else if (attr == R.styleable.UISkinDef_ui_skin_separator_bottom) {
                builder.bottomSeparator(id);
            } else if (attr == R.styleable.UISkinDef_ui_skin_separator_left) {
                builder.leftSeparator(id);
            }else if(attr == R.styleable.UISkinDef_ui_skin_bg_tint_color) {
                builder.bgTintColor(id);
            }else if(attr == R.styleable.UISkinDef_ui_skin_progress_color){
                builder.progressColor(id);
            }else if(attr == R.styleable.UISkinDef_ui_skin_underline){
                builder.underline(id);
            }else if(attr == R.styleable.UISkinDef_ui_skin_more_bg_color){
                builder.moreBgColor(id);
            }else if(attr == R.styleable.UISkinDef_ui_skin_more_text_color){
                builder.moreTextColor(id);
            }else if(attr == R.styleable.UISkinDef_ui_skin_hint_color){
                builder.hintColor(id);
            }else if(attr == R.styleable.UISkinDef_ui_skin_text_compound_tint_color){
                builder.textCompoundTintColor(id);
            }else if(attr == R.styleable.UISkinDef_ui_skin_text_compound_src_left){
                builder.textCompoundLeftSrc(id);
            }else if(attr == R.styleable.UISkinDef_ui_skin_text_compound_src_top){
                builder.textCompoundTopSrc(id);
            }else if(attr == R.styleable.UISkinDef_ui_skin_text_compound_src_right){
                builder.textCompoundRightSrc(id);
            }else if(attr == R.styleable.UISkinDef_ui_skin_text_compound_src_bottom){
