 ...
 * <li>每个 Tab 都可以非常灵活的配置，如果没有提供相关配置，则使用 UIBasicTabSegment 提供的配置，具体参考 {@link UITab}</li>
 * </ul>
 * <p>
 * <h3>使用case: </h3>
 * <ul>
 ...
 * <code>
 * UIBasicTabSegment mTabSegment = new UIBasicTabSegment((getContext());
 * // config mTabSegment
 ...
 * @date 2016-01-27
 */
public class UIBasicTabSegment extends HorizontalScrollView implements IUILayout, IUISkinHandlerView, IUISkinDefaultAttrProvider {

    private static final String TAG = "UIBasicTabSegment";
    ...
    protected UITabBuilder mTabBuilder;
    ...
    static {
        ...
        sDefaultSkinAttrs.put(UISkinValueBuilder.BOTTOM_SEPARATOR, R.attr.ui_skin_support_tab_separator_color);
        sDefaultSkinAttrs.put(UISkinValueBuilder.TOP_SEPARATOR, R.attr.ui_skin_support_tab_separator_color);
        sDefaultSkinAttrs.put(UISkinValueBuilder.BACKGROUND, R.attr.ui_skin_support_tab_bg);
    }

    public UIBasicTabSegment(Context context) {
        this(context, null);
    }

    public UIBasicTabSegment(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.UITabSegmentStyle);
    }

    public UIBasicTabSegment(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mSelectNoAnimation = array.getBoolean(R.styleable.UITabSegment_ui_tab_select_no_animation, false);
        array.recycle();
    ...
    public void setDefaultTabIconPosition(@UITab.IconPosition int iconPosition) {
     ...
     */
    public UIBasicTabSegment addTab(UITab tab) {
    ...
    private void layoutIndicator(UITab model, boolean invalidate) {
        ...
        mIndicator.updateInfo(model.contentLeft, model.contentWidth, model.selectedColorAttr == 0 ? model.selectColor : UISkinHelper.getSkinColor(this, model.selectedColorAttr), 0f);
        if (invalidate) {
            mContentLayout.invalidate();
        }
    }
    ...
    private void layoutIndicatorInTransition(UITab preModel, UITab targetModel, float offsetPercent) {
        ...
        int indicatorColor = UIColorHelper.computeColor(
                preModel.selectedColorAttr == 0 ? preModel.selectColor : UISkinHelper.getSkinColor(this, preModel.selectedColorAttr),
                targetModel.selectedColorAttr == 0 ? targetModel.selectColor : UISkinHelper.getSkinColor(this, targetModel.selectedColorAttr),
                offsetPercent);
    ...
    public interface TabBuilderUpdater {
        void update(UITabBuilder builder);
    }
}
